...
static gchar *met_parse(const gchar *meto, gsize len) {
  gchar *p;
  gchar *rp;
...
static void met_finish(GObject *source, GAsyncResult *result, gpointer data) {
...
void metoffice_start_open(WeatherInfo *info) {
  ...
  loc = info->location;
  url = g_strdup_printf("http://www.metoffice.gov.uk/weather/europe/uk/%s.html",
                        loc->zone + 1);
