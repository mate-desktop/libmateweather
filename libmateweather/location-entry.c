...
G_DEFINE_TYPE(MateWeatherLocationEntry, mateweather_location_entry,
              GTK_TYPE_ENTRY)
...
static void mateweather_location_entry_init(MateWeatherLocationEntry *entry) {
  ...
  gtk_entry_completion_set_text_column(
      completion, MATEWEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME);
  gtk_entry_completion_set_match_func(completion, matcher, NULL, NULL);

  g_signal_connect(completion, "match_selected", G_CALLBACK(match_selected),
                   entry);
...
static void mateweather_location_entry_class_init(
    MateWeatherLocationEntryClass *location_entry_class) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_pointer("top", "Top Location",
                           ...
                           "used to fill in the entry",
                           G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      object_class, PROP_LOCATION,
      g_param_spec_pointer("location", "Location",
...
static void set_location_internal(MateWeatherLocationEntry *entry,
                                  GtkTreeModel *model, GtkTreeIter *iter) {
  ...
  if (iter) {
    gtk_tree_model_get(model, iter, MATEWEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME,
                       &name, MATEWEATHER_LOCATION_ENTRY_COL_LOCATION, &loc,
                       -1);
...
static void fill_location_entry_model(GtkTreeStore *store,
                                      MateWeatherLocation *loc,
  ...
  case MATEWEATHER_LOCATION_ADM2:
    ...
    for (i = 0; children[i]; i++) {
      fill_location_entry_model(store, children[i], parent_display_name,
                                parent_compare_name);
    }
    break;
  ...
  case MATEWEATHER_LOCATION_ADM1:
    ...
    display_name = g_strdup_printf("%s, %s", mateweather_location_get_name(loc),
                                   parent_display_name);
    compare_name = g_strdup_printf(
        "%s, %s", mateweather_location_get_sort_name(loc), parent_compare_name);

    for (i = 0; children[i]; i++) {
      fill_location_entry_model(store, children[i], display_name, compare_name);
    }
    ...
    if (children[0] && children[1]) {
      ...
      for (i = 0; children[i]; i++) {
        display_name = g_strdup_printf(
            "%s (%s), %s", mateweather_location_get_name(loc),
            mateweather_location_get_name(children[i]), parent_display_name);
        compare_name = g_strdup_printf(
            "%s (%s), %s", mateweather_location_get_sort_name(loc),
            mateweather_location_get_sort_name(children[i]),
            parent_compare_name);

        gtk_tree_store_append(store, &iter, NULL);
        gtk_tree_store_set(
            store, &iter, MATEWEATHER_LOCATION_ENTRY_COL_LOCATION, children[i],
            MATEWEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME, display_name,
            MATEWEATHER_LOCATION_ENTRY_COL_COMPARE_NAME, compare_name, -1);
       ...
       */
      display_name = g_strdup_printf(
          "%s, %s", mateweather_location_get_name(loc), parent_display_name);
      compare_name =
          g_strdup_printf("%s, %s", mateweather_location_get_sort_name(loc),
                          parent_compare_name);

      gtk_tree_store_append(store, &iter, NULL);
      gtk_tree_store_set(
          store, &iter, MATEWEATHER_LOCATION_ENTRY_COL_LOCATION, children[0],
          MATEWEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME, display_name,
          MATEWEATHER_LOCATION_ENTRY_COL_COMPARE_NAME, compare_name, -1);
  ...
  case MATEWEATHER_LOCATION_WEATHER_STATION:
    ...
    gtk_tree_store_append(store, &iter, NULL);
    gtk_tree_store_set(
        store, &iter, MATEWEATHER_LOCATION_ENTRY_COL_LOCATION, loc,
        MATEWEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME, display_name,
        MATEWEATHER_LOCATION_ENTRY_COL_COMPARE_NAME, compare_name, -1);
...
mateweather_location_entry_build_model(MateWeatherLocationEntry *entry,
  ...
  store = gtk_tree_store_new(4, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_STRING,
                             G_TYPE_STRING);
...
static char *find_word(const char *full_name, const char *word, int word_len,
...
static gboolean matcher(GtkEntryCompletion *completion, const char *key,
  ...
  gtk_tree_model_get(gtk_entry_completion_get_model(completion), iter,
                     ...
                     MATEWEATHER_LOCATION_ENTRY_COL_LOCATION, &loc, -1);
  name = name_mem;
...
GtkWidget *mateweather_location_entry_new(MateWeatherLocation *top) {
  return g_object_new(MATEWEATHER_TYPE_LOCATION_ENTRY, "top", top, NULL);
}
