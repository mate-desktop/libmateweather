...
const char *mateweather_dpgettext(const char *context, const char *str) {
  mateweather_gettext_init();
...
WeatherLocation *weather_location_new(const gchar *name, const gchar *code,
  ...
  location->latlon_valid =
      (location->latitude < DBL_MAX && location->longitude < DBL_MAX);
...
WeatherLocation *weather_location_clone(const WeatherLocation *location) {
  ...
  clone = weather_location_new(location->name, location->code, location->zone,
...
static const gchar *wind_direction_str[] = {
    N_("Variable"),          N_("North"),
    N_("North - NorthEast"), N_("Northeast"),
    N_("East - NorthEast"),  N_("East"),
    N_("East - Southeast"),  N_("Southeast"),
    N_("South - Southeast"), N_("South"),
    N_("South - Southwest"), N_("Southwest"),
    N_("West - Southwest"),  N_("West"),
    N_("West - Northwest"),  N_("Northwest"),
    N_("North - Northwest")};
...
static const gchar *sky_str[] = {N_("Clear Sky"), N_("Broken clouds"),
                                 N_("Scattered clouds"), N_("Few clouds"),
                                 N_("Overcast")};
 ...
 */
/*                   NONE                         VICINITY LIGHT MODERATE HEAVY
 * SHALLOW                      PATCHES                         PARTIAL
 * THUNDERSTORM                    BLOWING                      SHOWERS DRIFTING
 * FREEZING                      */
/*               *******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************/
static const gchar *conditions_str[24][13] = {
    ...
    /* NONE          */ {"??", "??", "??", "??", "??", "??", "??", "??",
                         N_("Thunderstorm"), "??", "??", "??", "??"},
    /* DRIZZLE       */
    {N_("Drizzle"), "??", N_("Light drizzle"), N_("Moderate drizzle"),
     N_("Heavy drizzle"), "??", "??", "??", "??", "??", "??", "??",
     N_("Freezing drizzle")},
    /* RAIN          */
    {N_("Rain"), "??", N_("Light rain"), N_("Moderate rain"), N_("Heavy rain"),
     "??", "??", "??", N_("Thunderstorm"), "??", N_("Rain showers"), "??",
     N_("Freezing rain")},
    /* SNOW          */
    {N_("Snow"), "??", N_("Light snow"), N_("Moderate snow"), N_("Heavy snow"),
     "??", "??", "??", N_("Snowstorm"), N_("Blowing snowfall"),
     N_("Snow showers"), N_("Drifting snow"), "??"},
    /* SNOW_GRAINS   */
    {N_("Snow grains"), "??", N_("Light snow grains"),
     N_("Moderate snow grains"), N_("Heavy snow grains"), "??", "??", "??",
     "??", "??", "??", "??", "??"},
    /* ICE_CRYSTALS  */
    {N_("Ice crystals"), "??", "??", N_("Ice crystals"), "??", "??", "??", "??",
     "??", "??", "??", "??", "??"},
    /* ICE_PELLETS   */
    {N_("Ice pellets"), "??", N_("Few ice pellets"), N_("Moderate ice pellets"),
     N_("Heavy ice pellets"), "??", "??", "??", N_("Ice pellet storm"), "??",
    ...
    /* SMALL_HAIL    */
    {N_("Small hail"), "??", "??", N_("Small hail"), "??", "??", "??", "??",
     N_("Small hailstorm"), "??", N_("Showers of small hail"), "??", "??"},
    /* PRECIPITATION */
    ...
    {N_("Mist"), "??", "??", N_("Mist"), "??", "??", "??", "??", "??", "??",
     "??", "??", "??"},
    /* FOG           */
    {N_("Fog"), N_("Fog in the vicinity"), "??", N_("Fog"), "??",
     N_("Shallow fog"), N_("Patches of fog"), N_("Partial fog"), "??", "??",
     "??", "??", N_("Freezing fog")},
    /* SMOKE         */
    {N_("Smoke"), "??", "??", N_("Smoke"), "??", "??", "??", "??", "??", "??",
     "??", "??", "??"},
    /* VOLCANIC_ASH  */
    {N_("Volcanic ash"), "??", "??", N_("Volcanic ash"), "??", "??", "??", "??",
     "??", "??", "??", "??", "??"},
    /* SAND          */
    {N_("Sand"), "??", "??", N_("Sand"), "??", "??", "??", "??", "??",
     N_("Blowing sand"), "", N_("Drifting sand"), "??"},
    /* HAZE          */
    {N_("Haze"), "??", "??", N_("Haze"), "??", "??", "??", "??", "??", "??",
     "??", "??", "??"},
    /* SPRAY         */
    ...
    /* DUST          */
    {N_("Dust"), "??", "??", N_("Dust"), "??", "??", "??", "??", "??",
     N_("Blowing dust"), "??", N_("Drifting dust"), "??"},
    /* SQUALL        */
    {N_("Squall"), "??", "??", N_("Squall"), "??", "??", "??", "??", "??", "??",
     "??", "??", "??"},
    /* SANDSTORM     */
    {N_("Sandstorm"), N_("Sandstorm in the vicinity"), "??", N_("Sandstorm"),
     N_("Heavy sandstorm"), "??", "??", "??", "??", "??", "??", "??", "??"},
    /* DUSTSTORM     */
    {N_("Duststorm"), N_("Duststorm in the vicinity"), "??", N_("Duststorm"),
     N_("Heavy duststorm"), "??", "??", "??", "??", "??", "??", "??", "??"},
    /* FUNNEL_CLOUD  */
    {N_("Funnel cloud"), "??", "??", "??", "??", "??", "??", "??", "??", "??",
     "??", "??", "??"},
    ...
    {N_("Tornado"), "??", "??", "??", "??", "??", "??", "??", "??", "??", "??",
     "??", "??"},
    /* DUST_WHIRLS   */
    {N_("Dust whirls"), N_("Dust whirls in the vicinity"), "??",
     N_("Dust whirls"), "??", "??", "??", "??", "??", "??", "??", "??", "??"}};
...
const gchar *weather_conditions_string(WeatherConditions cond) {
  ...
  if (!cond.significant) {
    ...
    if (cond.phenomenon > PHENOMENON_INVALID &&
        ...
        cond.qualifier > QUALIFIER_INVALID && cond.qualifier < QUALIFIER_LAST)
 ...
 * <Olof.Oberg@modopaper.modogroup.com> */
...
static inline gdouble calc_apparent(WeatherInfo *info) {
   ...
   */
    ...
    if (info->temp >= -500. && info->dew >= -500.) {
      ...
      apparent = 16.923 + 0.185212 * temp + 5.37941 * humidity -
                 0.100254 * temp * humidity + 9.41695e-3 * t2 +
                 7.28898e-3 * h2 + 3.45372e-4 * t2 * humidity -
                 8.14971e-4 * temp * h2 + 1.02102e-5 * t2 * h2 -
                 3.8646e-5 * t3 + 2.91583e-5 * h3 + 1.42721e-6 * t3 * humidity +
                 1.97483e-7 * temp * h3 - 2.18429e-8 * t3 * h2 +
                 8.43296e-10 * t2 * h3 - 4.81975e-11 * t3 * h3;
#else
      /*
       * An often cited alternative: values are within 5 degrees for
       * most ranges between 10% and 70% humidity and to 110 degrees.
       */
      apparent = -42.379 + 2.04901523 * temp + 10.14333127 * humidity -
                 0.22475541 * temp * humidity - 6.83783e-3 * t2 -
                 5.481717e-2 * h2 + 1.22874e-3 * t2 * humidity +
                 8.5282e-4 * temp * h2 - 1.99e-6 * t2 * h2;
#endif
    }
  } else {
    apparent = temp;
...
WeatherInfo *_weather_info_fill(WeatherInfo *info, WeatherLocation *location,
                                const WeatherPrefs *prefs, WeatherInfoFunc cb,
                                gpointer data) {
  g_return_val_if_fail(((info == NULL) && (location != NULL)) ||
                           ((info != NULL) && (location == NULL)),
                       NULL);
  ...
  info->radar_url = prefs->radar && prefs->radar_custom_url
                        ? g_strdup(prefs->radar_custom_url)
                        : NULL;
  info->finish_cb = cb;
  info->cb_data = data;
...
WeatherInfo *weather_info_clone(const WeatherInfo *info) {
  ...
  if (info->forecast_list) {
    ...
    for (p = info->forecast_list; p; p = p->next) {
      clone->forecast_list =
          g_slist_prepend(clone->forecast_list, weather_info_clone(p->data));
    }
...
const gchar *weather_info_get_update(WeatherInfo *info) {
  ...
  if (info->update != 0) {
     ...
     */
    timeformat =
        g_locale_from_utf8(_("%a, %b %d / %H:%M"), -1, NULL, NULL, NULL);
    if (!timeformat) {
      strcpy(buf, "???");
...
static const gchar *temperature_string(gdouble temp, TempUnit to_unit,
                                       gboolean want_round) {
  ...
  case TEMP_UNIT_FAHRENHEIT:
    if (!want_round) {
      ...
      if (fetestexcept(range_problem))
        ...
        /* Translators: This is the temperature in degrees Fahrenheit (\302\260
  ...
  case TEMP_UNIT_CENTIGRADE:
    if (!want_round) {
      /* Translators: This is the temperature in degrees Celsius (\302\260 is
       * U+00B0 DEGREE SIGN) */
...
const gchar *weather_info_get_pressure(WeatherInfo *info) {
  ...
  case PRESSURE_UNIT_MM_HG:
    ...
    g_snprintf(buf, sizeof(buf), _("%.1f mmHg"),
               PRESSURE_INCH_TO_MM(info->pressure));
    break;
  case PRESSURE_UNIT_KPA:
    /* Translators: This is pressure in kiloPascals */
    g_snprintf(buf, sizeof(buf), _("%.2f kPa"),
               PRESSURE_INCH_TO_KPA(info->pressure));
    break;
  case PRESSURE_UNIT_HPA:
    /* Translators: This is pressure in hectoPascals */
    g_snprintf(buf, sizeof(buf), _("%.2f hPa"),
               PRESSURE_INCH_TO_HPA(info->pressure));
    break;
  case PRESSURE_UNIT_MB:
    /* Translators: This is pressure in millibars */
    g_snprintf(buf, sizeof(buf), _("%.2f mb"),
               PRESSURE_INCH_TO_MB(info->pressure));
    break;
  case PRESSURE_UNIT_ATM:
    /* Translators: This is pressure in atmospheres */
    g_snprintf(buf, sizeof(buf), _("%.3f atm"),
               PRESSURE_INCH_TO_ATM(info->pressure));
    break;
...
const gchar *weather_info_get_visibility(WeatherInfo *info) {
  ...
  case DISTANCE_UNIT_KM:
    ...
    g_snprintf(buf, sizeof(buf), _("%.1f km"),
               VISIBILITY_SM_TO_KM(info->visibility));
    break;
  case DISTANCE_UNIT_METERS:
    /* Translators: This is the visibility in meters */
    g_snprintf(buf, sizeof(buf), _("%.0fm"),
               VISIBILITY_SM_TO_M(info->visibility));
    break;
...
const gchar *weather_info_get_icon_name(WeatherInfo *info) {
  ...
  if (info->midnightSun || (!info->sunriseValid && !info->sunsetValid))
    daytime = TRUE;
  else if (info->polarNight)
    daytime = FALSE;
  else {
    current_time = time(NULL);
    daytime = (!info->sunriseValid || (current_time >= info->sunrise)) &&
   ...
   */
  if (info->moonValid &&
      weather_info_get_value_moonphase(info, &moonPhase, &moonLat)) {
    ...
    if ((0 == (MOON_PHASES & 0x1)) && (MOON_PHASES / 2 != phase)) {
      g_snprintf(icon, sizeof(icon_buffer) - strlen(icon_buffer), "-%03d",
                 phase * 360 / MOON_PHASES);
    }
  }
  return icon_buffer;
}

static gboolean temperature_value(gdouble temp_f, TempUnit to_unit,
                                  gdouble *value, TempUnit def_unit) {
  gboolean ok = TRUE;
...
static gboolean speed_value(gdouble knots, SpeedUnit to_unit, gdouble *value,
                            SpeedUnit def_unit) {
  gboolean ok = TRUE;
...
static gboolean pressure_value(gdouble inHg, PressureUnit to_unit,
                               gdouble *value, PressureUnit def_unit) {
  gboolean ok = TRUE;
...
static gboolean distance_value(gdouble miles, DistanceUnit to_unit,
                               gdouble *value, DistanceUnit def_unit) {
  gboolean ok = TRUE;
...
gboolean
weather_info_get_value_conditions(WeatherInfo *info,
...
gboolean weather_info_get_value_temp(WeatherInfo *info, TempUnit unit,
                                     gdouble *value) {
  g_return_val_if_fail(info != NULL, FALSE);
...
gboolean weather_info_get_value_temp_min(WeatherInfo *info, TempUnit unit,
                                         gdouble *value) {
  g_return_val_if_fail(info != NULL, FALSE);
  g_return_val_if_fail(value != NULL, FALSE);
...
gboolean weather_info_get_value_temp_max(WeatherInfo *info, TempUnit unit,
                                         gdouble *value) {
  g_return_val_if_fail(info != NULL, FALSE);
  g_return_val_if_fail(value != NULL, FALSE);
...
gboolean weather_info_get_value_dew(WeatherInfo *info, TempUnit unit,
                                    gdouble *value) {
  g_return_val_if_fail(info != NULL, FALSE);
  g_return_val_if_fail(value != NULL, FALSE);
...
gboolean weather_info_get_value_apparent(WeatherInfo *info, TempUnit unit,
                                         gdouble *value) {
  g_return_val_if_fail(info != NULL, FALSE);
  g_return_val_if_fail(value != NULL, FALSE);
  ...
  return temperature_value(calc_apparent(info), unit, value,
...
gboolean weather_info_get_value_moonphase(WeatherInfo *info,
...
gboolean weather_info_get_value_wind(WeatherInfo *info, SpeedUnit unit,
                                     gdouble *speed,
  ...
  if (info->windspeed < 0.0 || info->wind <= WIND_INVALID ||
      info->wind >= WIND_LAST)
    return FALSE;
...
gboolean weather_info_get_value_pressure(WeatherInfo *info, PressureUnit unit,
                                         gdouble *value) {
  g_return_val_if_fail(info != NULL, FALSE);
  g_return_val_if_fail(value != NULL, FALSE);
...
gboolean weather_info_get_value_visibility(WeatherInfo *info, DistanceUnit unit,
                                           gdouble *value) {
  g_return_val_if_fail(info != NULL, FALSE);
  g_return_val_if_fail(value != NULL, FALSE);
 ...
 */
gboolean weather_info_get_upcoming_moonphases(WeatherInfo *info,
                                              time_t *phases) {
  g_return_val_if_fail(info != NULL, FALSE);
