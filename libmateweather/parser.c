...
MateWeatherParser *mateweather_parser_new(gboolean use_regions) {
  ...
  for (i = 0; parser->locales[i] != NULL; i++) {
    filename =
        g_strdup_printf("%s/Locations.%s.xml", MATEWEATHER_XML_LOCATION_DIR,
                        parser->locales[i]);
    ...
    filename =
        g_strdup_printf("%s/Locations.%s.xml.gz", MATEWEATHER_XML_LOCATION_DIR,
                        parser->locales[i]);
  ...
  if (!filename)
    filename =
        g_build_filename(MATEWEATHER_XML_LOCATION_DIR, "Locations.xml", NULL);

  if (!g_file_test(filename, G_FILE_TEST_IS_REGULAR) && zlib_support) {
    g_free(filename);
    filename = g_build_filename(MATEWEATHER_XML_LOCATION_DIR,
                                "Locations.xml.gz", NULL);
  }
