...
static gboolean mateweather_xml_parse_node(MateWeatherLocation *gloc,
                                           GtkTreeStore *store,
                                           GtkTreeIter *parent) {
  ...
  case MATEWEATHER_LOCATION_ADM1:
    ...
    gtk_tree_store_set(store, &iter, MATEWEATHER_XML_COL_LOC, name, -1);
    break;
  ...
  case MATEWEATHER_LOCATION_CITY:
    ...
    if (children[0] && !children[1]) {
      ...
      gtk_tree_store_set(store, &iter, MATEWEATHER_XML_COL_POINTER, wloc, -1);
    }
    break;
  ...
  case MATEWEATHER_LOCATION_WEATHER_STATION:
    ...
    if (parent_loc &&
        mateweather_location_get_level(parent_loc) == MATEWEATHER_LOCATION_CITY)
...
static gboolean free_locations(GtkTreeModel *model, GtkTreePath *path,
  ...
  if (loc) {
    ...
    gtk_tree_store_set((GtkTreeStore *)model, iter, MATEWEATHER_XML_COL_POINTER,
                       NULL, -1);
  }
...
/* Frees model returned from @mateweather_xml_load_locations. It contains
   allocated WeatherLocation-s, thus this takes care of the freeing of that
   memory. */
