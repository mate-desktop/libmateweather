...
/**
 *  Humans don't deal well with .MONDAY...SUNNY AND BLAH BLAH.TUESDAY...THEN
 * THIS AND THAT.WEDNESDAY...RAINY BLAH BLAH. This function makes it easier to
 * read.
 */
...
static gboolean hasAttr(xmlNode *node, const char *attr_name,
...
static GSList *parseForecastXml(const char *buff, gsize len,
                                WeatherInfo *master_info) {
  GSList *res = NULL;
  xmlDocPtr doc;
  xmlNode *root, *node;
      ...
      for (n = node->children; n; n = n->next) {
        ...
        if (isElem(n, "time-layout")) {
          if (!time_layout && hasAttr(n, "summarization", "24hourly")) {
            ...
            for (c = n->children; c && (count < 7 || !time_layout);
                 c = c->next) {
              if (c->name && !time_layout && isElem(c, "layout-key")) {
                ...
                if (val) {
                  GDateTime *dt =
                      g_date_time_new_from_iso8601((const char *)val, NULL);
                  if (dt != NULL) {
          ...
          for (p = n->children; p; p = p->next) {
            if (p->name && isElem(p, "temperature") &&
                hasAttr(p, "time-layout", time_layout)) {
              xmlNode *c;
              GSList *at = res;
              ...
              for (c = p->children; c && at; c = c->next) {
                if (isElem(c, "value")) {
                  ...
                  nfo->tempMinMaxValid =
                      nfo->tempMinMaxValid ||
                      (nfo->temp_max > -999.0 && nfo->temp_min > -999.0);
                  nfo->valid = nfo->tempMinMaxValid;

                  at = at->next;
                }
              }
            } else if (p->name && isElem(p, "weather") &&
                       hasAttr(p, "time-layout", time_layout)) {
              xmlNode *c;
              GSList *at = res;
              ...
              for (c = p->children; c && at; c = c->next) {
                if (c->name && isElem(c, "weather-conditions")) {
                  ...
                  if (val && nfo) {
                    /* Checking from top to bottom, if 'value' contains 'name',
                       then that win, thus put longer (more precise) values to
                       the top. */
                    int i;
                    struct _ph_list {
                      const char *name;
                    ...
                    struct _sky_list {
                      const char *name;
                      WeatherSky sky;
                    } sky_list[] = {{"Mostly Sunny", SKY_BROKEN},
...
static void iwin_finish(GObject *source, GAsyncResult *result, gpointer data) {
...
void iwin_start_open(WeatherInfo *info) {
  ...
  if (loc->zone[0] == '-' &&
      (info->forecast_type != FORECAST_LIST || !loc->latlon_valid))
    return;
  ...
  if (info->forecast_type == FORECAST_LIST) {
    ...
    if (loc->latlon_valid) {
      ...
      url =
          g_strdup_printf("http://www.weather.gov/forecasts/xml/"
                          "sample_products/browser_interface/"
                          "ndfdBrowserClientByDay.php?&lat=%.02f&lon=%.02f&"
                          "format=24+hourly&startDate=%04d-%02d-%02d&numDays=7",
                          RADIANS_TO_DEGREES(loc->latitude),
                          RADIANS_TO_DEGREES(loc->longitude), year, month, day);
  ...
  ** file (the PA stands for the state pennsylvania). The url used wants the
  *state
  ...
  */
  ...
  url = g_strdup_printf(
      "http://tgftp.nws.noaa.gov/data/forecasts/zone/%s/%s.txt", state, zone);
