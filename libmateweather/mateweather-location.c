...
static int sort_locations_by_name(gconstpointer a, gconstpointer b) {
...
static int sort_locations_by_distance(gconstpointer a, gconstpointer b,
                                      gpointer user_data) {
  MateWeatherLocation *loc_a = *(MateWeatherLocation **)a;
  MateWeatherLocation *loc_b = *(MateWeatherLocation **)b;
...
static gboolean parse_coordinates(const char *coordinates, double *latitude,
                                  double *longitude) {
  char *p;
...
static char *unparse_coordinates(double latitude, double longitude) {
  ...
  return g_strdup_printf("%02d-%02d-%02d%c %03d-%02d-%02d%c", lat_d, lat_m,
                         lat_s, lat_dir, lon_d, lon_m, lon_s, lon_dir);
}
...
location_new_from_xml(MateWeatherParser *parser, MateWeatherLocationLevel level,
  ...
  while ((tagtype = xmlTextReaderNodeType(parser->xml)) !=
    ...
    if (!strcmp(tagname, "name") && !loc->name) {
        ...
        if (child->children) {
          for (i = 0; child->children[i]; i++)
            g_ptr_array_add(children,
                            mateweather_location_ref(child->children[i]));
        }
      ...
      child = location_new_from_xml(parser,
                                    MATEWEATHER_LOCATION_WEATHER_STATION, loc);
      if (!child)
        goto error_out;
      g_ptr_array_add(children, child);
...
double mateweather_location_get_distance(MateWeatherLocation *loc,
  ...
  return acos(cos(loc->latitude) * cos(loc2->latitude) *
                  cos(loc->longitude - loc2->longitude) +
              sin(loc->latitude) * sin(loc2->latitude)) *
         radius;
}
...
static void add_timezones(MateWeatherLocation *loc, GPtrArray *zones) {
  int i;
...
char *mateweather_location_get_city_name(MateWeatherLocation *loc) {
  ...
  else if (loc->level == MATEWEATHER_LOCATION_WEATHER_STATION && loc->parent &&
...
mateweather_location_to_weather_location(MateWeatherLocation *gloc,
  ...
  wloc = weather_location_new(name, code, zone, radar, coords,
                              mateweather_location_get_country(gloc), tz_hint);
  g_free(coords);
  return wloc;
}
