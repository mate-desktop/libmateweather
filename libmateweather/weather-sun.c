...
gdouble sunEclipLongitude(time_t t) {
   ...
   */
  longitude =
      fmod(DEGREES_TO_RADIANS(PERIGEE_LONGITUDE(ndays)) +
               2. * atan(sqrt((1. + e) / (1. - e)) * tan(eccenAnom / 2.)),
           2. * M_PI);
  if (longitude < 0.) {
    longitude += 2 * M_PI;
  }
...
static gdouble ecliptic_obliquity(gdouble time) {
  gdouble jc = EPOCH_TO_J2000(time) / (36525. * 86400.);
  gdouble eclip_secs = (84381.448 - (46.84024 * jc) - (59.e-5 * jc * jc) +
                        (1.813e-3 * jc * jc * jc));
 ...
 * declination (radians)
 */
void ecl2equ(gdouble time, gdouble eclipLon, gdouble eclipLat, gdouble *ra,
             gdouble *decl) {
  gdouble mEclipObliq = ecliptic_obliquity(time);

  if (ra) {
    *ra = RADIANS_TO_HOURS(atan2(
        (sin(eclipLon) * cos(mEclipObliq) - tan(eclipLat) * sin(mEclipObliq)),
        cos(eclipLon)));
    if (*ra < 0.)
      *ra += 24.;
  }
 ...
 */
static void gstObsv(gdouble ra, gdouble decl, gdouble obsLat, gdouble obsLon,
...
static gboolean calc_sun2(WeatherInfo *info, time_t t) {
  ...
  ecl2equ(lcl_midn + 86400., lambda + DEGREES_TO_RADIANS(SOL_PROGRESSION), 0.,
          &ra2, &decl2);
 ...
 * Returns: gboolean indicating if the results are valid.
 */
gboolean calc_sun(WeatherInfo *info) { return calc_sun_time(info, time(NULL)); }
...
gint weather_info_next_sun_event(WeatherInfo *info) {
  ...
  if (info->sunsetValid && (info->sunset > now) && (info->sunset < nxtEvent))
    nxtEvent = info->sunset;
  if (info->sunriseValid && (info->sunrise > now) && (info->sunrise < nxtEvent))
