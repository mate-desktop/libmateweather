...
static void deleted(GtkWidget *widget, GdkEvent *event, gpointer data) {
  gtk_main_quit();
}
...
static void location_changed(GObject *object, GParamSpec *param,
  ...
  if (zone)
    mateweather_timezone_menu_set_tzid(tzmenu,
...
int main(int argc, char **argv) {
  ...
  g_signal_connect(window, "delete-event", G_CALLBACK(deleted), NULL);
  ...
  g_signal_connect(entry, "notify::location", G_CALLBACK(location_changed),
                   combo);
