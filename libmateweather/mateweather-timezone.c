...
static gboolean parse_tzdata(const char *tzname, time_t start, time_t end,
  ...
  initial_offset = *(gint32 *)(ttinfos + initial_transition * TZ_TTINFO_SIZE +
                               TZ_TTINFO_GMTOFF_OFFSET);
  initial_offset = GINT32_FROM_BE(initial_offset);
  initial_isdst =
      *(ttinfos + initial_transition * TZ_TTINFO_SIZE + TZ_TTINFO_ISDST_OFFSET);

  if (second_transition != -1) {
    second_offset = *(gint32 *)(ttinfos + second_transition * TZ_TTINFO_SIZE +
                                TZ_TTINFO_GMTOFF_OFFSET);
    second_offset = GINT32_FROM_BE(second_offset);
    second_isdst = *(ttinfos + second_transition * TZ_TTINFO_SIZE +
...
static MateWeatherTimezone *parse_timezone(MateWeatherParser *parser) {
  ...
  if (parse_tzdata(id, parser->year_start, parser->year_end, &offset, &has_dst,
                   &dst_offset)) {
