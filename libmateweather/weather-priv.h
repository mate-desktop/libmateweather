...
struct _WeatherInfo {
  ...
  gchar *forecast;
  GSList *forecast_list; /* list of WeatherInfo* for the forecast, NULL if not
                            available */
  gchar *radar_buffer;
  gchar *radar_url;
...
#define MEAN_ECLIPTIC_LONGITUDE(d)                                             \
  (280.46457166 + (d) / 36525. * 35999.37244981)
...
void ecl2equ(gdouble t, gdouble eclipLon, gdouble eclipLat, gdouble *ra,
             gdouble *decl);
