  ...
  if (!strcmp(tokp, "CAVOK")) {
    ...
    if (pfrac) {
      if (*tokp == 'M') {
        ...
        den = atoi(sval);
        info->visibility = ((WeatherVisibility)num / ((WeatherVisibility)den));
...
static void metar_tok_temp(gchar *tokp, WeatherInfo *info) {
  ...
  *psep = 0;
  ...
  if (*pdew) {
    info->dew =
        (*pdew == 'M') ? TEMP_C_TO_F(-atoi(pdew + 1)) : TEMP_C_TO_F(atoi(pdew));
  } else {
    info->dew = -1000.0;
  }
}
...
static void metar_tok_cond(gchar *tokp, WeatherInfo *info) {
  ...
  if ((info->cond.qualifier != QUALIFIER_NONE) ||
      (info->cond.phenomenon != PHENOMENON_NONE))
...
#define VIS_RE_STR                                                             \
  ...
  "CAVOK"
#define COND_RE_STR                                                            \
  "(-|\\+)?(VC|MI|BC|PR|TS|BL|SH|DR|FZ)?(DZ|RA|SN|SG|IC|PE|GR|GS|UP|BR|FG|FU|" \
  "VA|SA|HZ|PY|DU|SQ|SS|DS|PO|\\+?FC)"
#define CLOUD_RE_STR                                                           \
  "((CLR|BKN|SCT|FEW|OVC|SKC|NSC)([0-9]{3}|///)?(CB|TCU|///)?)"
...
gboolean metar_parse(gchar *metar, WeatherInfo *info) {
  ...
  while (*p) {
    ...
    for (i = 0; i < RE_NUM && rm2.rm_so > 0; i++) {
      if (0 == regexec(&metar_re[i], p, 1, &rm, 0) && rm.rm_so < rm2.rm_so) {
        i2 = i;
...
static void metar_finish(GObject *source, GAsyncResult *result, gpointer data) {
...
void metar_start_open(WeatherInfo *info) {
  ...
  query = soup_form_encode("dataSource", "metars", "requestType", "retrieve",
                           "format", "xml", "hoursBeforeNow", "3", "mostRecent",
                           "true", "fields", "raw_text", "stationString",
                           loc->code, NULL);
  msg = soup_message_new_from_encoded_form(
      "GET", "https://aviationweather.gov/api/data/dataserver", query);
